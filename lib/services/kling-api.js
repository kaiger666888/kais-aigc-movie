...
export class KlingApiService {
    ...
    async pollTask(taskId) {
        ...
        while (Date.now() < deadline) {
            const res = await fetch(`${this.baseUrl}/v1/videos/text2video/${taskId}`, {
                headers: {
                    Authorization: `Bearer ${this.auth.get()}`,
                },
            });
    ...
    async submitAndDownload(prompt, outputDir, filename, options) {
        ...
        const klingOpts = {
            duration: String(options?.duration ?? options?.duration ?? 5),
            aspectRatio: options?.aspectRatio ?? options?.aspectRatio,
        ...
        if (!taskId) {
            throw new Error(`Kling submit failed after ${this.maxRetries} retries: ${lastError?.message}`);
        }
        // Poll for completion
...
//# sourceMappingURL=kling-api.js.map