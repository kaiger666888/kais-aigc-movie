 ...
 */
export class ComfyUIService implements VideoGenerator {
  ...
  private async loadWorkflow(params: {
    prompt: string;
    width: number;
    height: number;
    numFrames: number;
  ...
  private patchWorkflow(
    ...
    for (const node of Object.values(wf) as Array<Record<string, unknown>>) {
      const ct = node.class_type as string | undefined;
      if (!ct) continue;
      ...
      switch (ct) {
        ...
        case "EmptyHunyuanLatentVideo":
          ...
          if (typeof inputs.length === "number") inputs.length = params.numFrames;
          break;
        case "UNETLoader":
